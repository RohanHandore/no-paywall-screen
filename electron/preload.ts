...
const electronAPI = {
  ...
  onLanguageDetected: (callback: (language: string) => void) => {
    ...
    ipcRenderer.on("language-detected", subscription)
    return () => {
      ipcRenderer.removeListener("language-detected", subscription)
    }
  },
  ...
  generateNextResponse: (includeScreenshotContext?: boolean) => ipcRenderer.invoke("generate-next-response", includeScreenshotContext),
