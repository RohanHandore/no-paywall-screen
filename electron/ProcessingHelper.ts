...
export class ProcessingHelper {
  ...
  /**
   * Detects the programming language from screenshots using AI
   * Returns null if detection fails or is unavailable
   */
  private async detectLanguageFromScreenshots(
    ...
    try {
      ...
      // Supported languages mapping (normalize common variations)
      const supportedLanguages = [
        'python', 'javascript', 'java', 'golang', 'cpp', 'swift', 
        'kotlin', 'ruby', 'sql', 'r', 'csharp', 'typescript', 'rust'
      ];
      ...
      const languageMapping: { [key: string]: string } = {
        ...
        'js': 'javascript',
        'typescript': 'javascript', // Use javascript as fallback
        'ts': 'javascript',
        'java': 'java',
        'go': 'golang',
        'golang': 'golang',
        ...
        'csharp': 'csharp',
        'cs': 'csharp',
        'rust': 'javascript' // Use javascript as fallback
      };
      ...
      if (config.apiProvider === "openai" && this.openaiClient) {
        const messages = [
          {
            ...
            content: "You are a programming language identifier. Analyze the screenshot and identify the programming language being used. Respond with ONLY the language name in lowercase (e.g., 'python', 'javascript', 'java', 'cpp', 'golang', 'swift', 'kotlin', 'ruby', 'sql', 'r', 'csharp'). If uncertain, respond with 'unknown'."
          },
          {
            role: "user" as const,
            content: [
              {
                type: "text" as const,
                text: "Identify the programming language shown in these screenshots. Respond with ONLY the language name in lowercase, or 'unknown' if you cannot determine it."
              },
        ...
        if (responseText && responseText !== 'unknown') {
          const normalized = languageMapping[responseText] || responseText;
        ...
        const geminiMessages: GeminiMessage[] = [
          {
            ...
            parts: [
              {
                text: "Identify the programming language shown in these screenshots. Respond with ONLY the language name in lowercase (e.g., 'python', 'javascript', 'java', 'cpp', 'golang', 'swift', 'kotlin', 'ruby', 'sql', 'r', 'csharp'), or 'unknown' if you cannot determine it."
              },
              ...imageDataList.map(data => ({
                inlineData: {
                  mimeType: "image/png",
        ...
        if (responseData.candidates && responseData.candidates.length > 0) {
          const responseText = responseData.candidates[0].content.parts[0].text.toLowerCase().trim();
        ...
        const responseText = (response.content[0] as { type: 'text', text: string }).text.toLowerCase().trim();
      ...
      console.error("Error detecting language from screenshots:", error);
      return null;
    }
  }
  ...
  private async processScreenshotsHelper(
    ...
    try {
      ...
      // Step 0: Detect programming language from screenshots
      if (mainWindow) {
      ...
      let language = await this.getLanguage(); // Get current language as fallback
      const detectedLanguage = await this.detectLanguageFromScreenshots(screenshots, signal);
      
      if (detectedLanguage) {
        language = detectedLanguage;
        // Update config with detected language
        configHelper.updateConfig({ language: detectedLanguage });
        
        // Notify frontend to update the language display
        if (mainWindow) {
          try {
            await this.waitForInitialization(mainWindow);
            ...
            console.log(`Language automatically detected: ${detectedLanguage}`);
          } catch (error) {
            console.warn("Failed to update language in frontend:", error);
          }
        }
      }
